...
public class CriteriaApiEvent {
    private final EntityManager entityManager;

    public List<Event> findEvents(List<Long> users, List<EventState> states,
        ...
        if (users != null) {
            predicates.add(criteriaBuilder.and(eventRoot.get("initiator").get("id").in(users)));
        }

        if (categories != null) {
            predicates.add(criteriaBuilder.and(eventRoot.get("category").get("id").in(categories)));
        }

        if (states != null) {
            predicates.add(criteriaBuilder.and(states.stream()
        ...
        if (rangeStart != null && rangeEnd != null) {
            ...
            predicates.add(criteriaBuilder.and(criteriaBuilder.between(eventRoot.get("eventDate"), start, end)));
        }

        return entityManager.createQuery(eventCriteriaQuery.select(eventRoot)
                .where(predicates.toArray(Predicate[]::new))).getResultList();
    }
}
